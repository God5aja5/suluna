...
function AISandboxPage() {
  ...
  const createSandbox = async (fromHomeScreen = false) => {
    ...
    try {
      ...
      // Handle local mode when no E2B API key is provided
      if (data.localMode) {
        console.log('[createSandbox] Running in local mode - no live preview available');
        sandboxCreationRef.current = false;
        setLoading(false);
        setShowLoadingBackground(false);
        updateStatus('Local mode - Code generation only', true);
        setStructureContent('Local mode: Generated code will be shown without live preview.');
        
        // Add informational message
        addChatMessage('Running in local mode. To enable live website previews, add your E2B API key to secrets. Get your free API key at https://e2b.dev', 'system');
        return data;
      }
...
//   };
  ...
  }, [addChatMessage, log, sandboxData]);
