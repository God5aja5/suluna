...
console.log('[generate-ai-code-stream] AI config:', {
...
const openRouterAI = createOpenRouter({
...
export async function POST(request: NextRequest) {
  try {
    const { prompt, model = 'google/gemini-2.0-flash-exp', context, isEdit = false, customApiKey } = await request.json();
...
5. Make the requested change immediately`;
        ...
        // Determine provider from app config or custom model
        let modelConfig = appConfig.ai.modelApiConfig[model as keyof typeof appConfig.ai.modelApiConfig];
        let isCustomModel = false;
        
        // If model not in config, treat as custom OpenRouter model
        if (!modelConfig) {
          if (!customApiKey) {
            throw new Error(`Custom model ${model} requires an API key`);
          }
          isCustomModel = true;
          modelConfig = {
            provider: 'openrouter',
        ...
        if (provider === 'google' && !process.env.GEMINI_API_KEY) {
          throw new Error('GEMINI_API_KEY is required for Google models');
        }
        if (provider === 'openrouter' && !customApiKey && !process.env.OPENROUTER_API_KEY) {
          throw new Error('OPENROUTER_API_KEY is required for OpenRouter models');
        }
        
        if (provider === 'google') {
          modelProvider = googleGenerativeAI;
          actualModel = modelConfig.model.replace('google/', '');
          providerName = 'Google';
        } else if (provider === 'openrouter') {
          // Use custom API key if provided, otherwise use environment key
          const apiKey = customApiKey || process.env.OPENROUTER_API_KEY;
          if (isCustomModel) {
            // Create a new OpenRouter instance with custom API key
            ...
            modelProvider = openRouterAI;
          }
          actualModel = modelConfig.model; // OpenRouter uses full model names
...
- Focus on clean, modern website generation without external scraping dependencies
...
It's better to have 3 complete files than 10 incomplete files.`
        ...
        // Add temperature based on provider
        if (provider === 'google') {
        ...
        while (retryCount <= maxRetries) {
          try {
            ...
            if (retryCount < maxRetries && isRetryableError) {
              ...
              await sendProgress({ 
                type: 'error', 
                message: `Failed to initialize Gemini streaming: ${streamError.message}` 
              });
              
              // Provide helpful info for Google model errors
              await sendProgress({ 
                type: 'info', 
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                // Use Google Generative AI for all completions
                const completionClient = googleGenerativeAI;
                
                // Use Gemini model for completion
                const completionModelName = model.startsWith('google/') ? model.slice('google/'.length) : 'gemini-1.5-flash';
