...
export async function POST() {
  try {
    ...
    if (!process.env.E2B_API_KEY) {
      console.log('[create-ai-sandbox-v2] No E2B API key found, running in local mode');
      return NextResponse.json({
        success: false,
        message: 'No E2B API key provided. App will run in local mode without live previews.',
        localMode: true
      });
    }
