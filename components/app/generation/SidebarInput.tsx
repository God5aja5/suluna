...
export default function SidebarInput({ onSubmit, disabled = false }: SidebarInputProps) {
  ...
  const [showCustomModelForm, setShowCustomModelForm] = useState<boolean>(false);
  const [customModelName, setCustomModelName] = useState<string>("");
  const [customApiKey, setCustomApiKey] = useState<string>("");
  const [customModels, setCustomModels] = useState<Array<{id: string, name: string, apiKey: string}>>([]);
  ...
  const defaultModels = appConfig.ai.availableModels.map((modelId: string) => ({
    id: modelId,
    name: appConfig.ai.modelDisplayNames[modelId] || modelId
  }));
  
  const allModels = [...defaultModels, ...customModels.map(cm => ({
    id: cm.id,
    name: cm.name
  }))];

  const handleAddCustomModel = () => {
    if (customModelName.trim() && customApiKey.trim()) {
      const newModel = {
        ...
        {isValidUrl && (
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              >
                {allModels.map((model: {id: string, name: string}) => (
            ...
            <div>
              <button
                ...
                disabled={disabled}
                className="w-full px-3 py-2 text-xs font-medium text-blue-600 bg-blue-50 rounded border border-blue-200 hover:bg-blue-100 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors"
              >
                {showCustomModelForm ? 'Cancel' : '+ Add Custom Model'}
              </button>
            </div>
            ...
            {showCustomModelForm && (
              <div className="border border-gray-200 rounded p-3 bg-gray-50 space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">OpenRouter Model Name</label>
                  <input
                    type="text"
                    value={customModelName}
                    onChange={(e) => setCustomModelName(e.target.value)}
                    placeholder="e.g., anthropic/claude-3-sonnet, mistralai/mistral-7b"
                    className="w-full px-2 py-1.5 text-xs text-gray-700 bg-white rounded border border-gray-200 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                ...
                <button
                  ...
                  disabled={!customModelName.trim() || !customApiKey.trim()}
                  className={`w-full py-1.5 px-3 rounded text-xs font-medium transition-all ${
                    customModelName.trim() && customApiKey.trim()
